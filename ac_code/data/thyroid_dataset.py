...
ThyroidXL 数据集（级联架构）：
  ...
  - ThyroidPatchDataset: 任务 C 分类，用 GT mask 外接框从原图裁出结节 patch（含 PTC 标签）。

共用：患者级划分（防泄漏）、PTC 标签读取（缺失自动生成默认）。
"""
import os
import csv
import json
...
# ---------------- PTC 标签 ----------------
def load_ptc_labels(cfg):
    p = cfg.ptc_labels_path()
    if not os.path.exists(p):
        from data.build_ptc_labels import build_ptc_labels
        build_ptc_labels(cfg.data_root, p)
    mapping = {}
    with open(p, "r", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            mapping[row["patient_id"]] = int(row["ptc"])
    return mapping


# ---------------- 样本清单（seg / patch 共用） ----------------
def _build_samples(cfg, split, require_ptc=False):
    """require_ptc=True 仅保留有 PTC 标签的样本（分类训练用）。"""
    splits = make_or_load_splits(cfg)
    pids = splits.get(split, [])
    mapping = load_ptc_labels(cfg)
    ...
    for pid in pids:
        for fp in sorted(glob.glob(os.path.join(img_dir, f"{pid}_*.png"))):
            ...
            if not os.path.exists(mpath):
                continue
            ptc = mapping.get(pid, -1)
            if require_ptc and ptc < 0:
                continue
            out.append(dict(
...
class ThyroidSegDataset(Dataset):
    def __init__(self, cfg, split="train", augment=None, transforms=None):
        ...
        self.samples = _build_samples(cfg, split, require_ptc=False)
...
class ThyroidPatchDataset(Dataset):
    ...
    def __init__(self, cfg, split="train", augment=None, transforms=None):
        ...
        self.mean = np.array(cfg.mean, dtype=np.float32).reshape(3, 1, 1)
        self.std = np.array(cfg.std, dtype=np.float32).reshape(3, 1, 1)
        self.samples = _build_samples(cfg, split, require_ptc=True)
