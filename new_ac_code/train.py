...
训练入口：级联两阶段（默认都跑）。
  ...
  阶段2 (cls)  : 用 GT mask 裁出结节 patch，训练任务 C 三分类器，保存 runs/cls_best.pth / cls_last.pth
用法：
...
三分类标签默认由 annotations 生成 cls_labels.csv；覆盖该文件即可用自有标签。
"""
import os
import sys
import time
...
from models.losses import seg_loss, ClsCE
from evaluate import evaluate_seg, _collate_seg, _collate_cls, _cls_quick_val


def compute_class_weights_patch(ds, num_classes=3):
    counts = [0] * num_classes
    for s in ds.samples:
        if 0 <= s["cls"] < num_classes:
            counts[s["cls"]] += 1
    total = sum(counts)
    if total == 0 or any(c == 0 for c in counts):
        return None
    # w_c = total / (C * count_c)，缓解类别不平衡
    w = [total / (num_classes * c) for c in counts]
...
def train_cls(cfg, device):
    ...
    model = build_classifier(patch_size=cfg.patch_size, backbone=cfg.cls_backbone,
                             ...
                             num_classes=cfg.cls_num_classes).to(device)
    n_params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"[cls] 参数 {n_params:.1f}M  patch_size={cfg.patch_size}  num_classes={cfg.cls_num_classes}")

    cw = compute_class_weights_patch(train_ds, cfg.cls_num_classes) if cfg.cls_class_weights else None
    if cw is not None:
        print(f"[cls] class_weight={cw.tolist()}")
    crit = ClsCE(cw)
