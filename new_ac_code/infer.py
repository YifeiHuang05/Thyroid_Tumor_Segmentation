...
推理（级联）：对每张图
  ...
  3) 三分类器 -> 各类概率（0=良性, 1=恶性乳头状, 2=恶性非乳头状）
输出：
  - out/masks/<name>.png                      预测 mask（原始尺寸）
  - out/cls_preds.csv                         patient_id, image, p_benign, p_mal_pap, p_mal_nonpap, cls_pred
    ...
    with torch.no_grad():
        for b in loader:
            ...
            for i in range(seg_prob.shape[0]):
                ...
                if bbox is None:
                    probs = np.zeros(cfg.cls_num_classes, dtype=float)
                else:
                    ...
                    probs = torch.softmax(cls_model(t), dim=1).cpu().numpy()[0].astype(float)
                pred_cls = int(np.argmax(probs))
                rows.append((b["patient_id"][i], b["name"][i],
                              f"{probs[0]:.4f}", f"{probs[1]:.4f}", f"{probs[2]:.4f}", pred_cls))

    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["patient_id", "image", "p_benign", "p_mal_pap", "p_mal_nonpap", "cls_pred"])
        w.writerows(rows)
    print(f"[infer] 完成：mask -> {mask_dir}  | 三分类 -> {csv_path}  (共 {len(rows)} 行)")
