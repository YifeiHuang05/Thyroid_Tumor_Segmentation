...
架构：级联（cascade）
    ...
    ->  任务 C 三分类器（输入结节 patch，输出 良性/恶性乳头状/恶性非乳头状 三类概率）
...
class Config:
    ...
    cls_labels: str = "cls_labels.csv"        # 三分类标签；缺失时由 build_cls_labels.py 生成默认
    ...
    # ---- 分类（任务 C，三分类） ----
    cls_num_classes: int = 3                   # 0=良性, 1=恶性乳头状, 2=恶性非乳头状
    cls_names: tuple = ("benign", "malignant_papillary", "malignant_nonpapillary")
...
def parse_overrides(argv_list, cfg: Config) -> Config:
    ...
    for f in ["data_root", "images_dir", "masks_dir", "annotations", "cls_labels",
              ...
              "base_channels", "margin", "patch_size", "cls_num_classes", "cls_backbone",
              "cls_use_timm", "cls_pretrained", "cls_class_weights",
