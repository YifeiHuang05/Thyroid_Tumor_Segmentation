...
【仅用组织学 Histopathology 判定，不使用 FNAC】（与“只取 506 例”一致）：
  PTC=1  若 Histopathology 含 "papillary thyroid"（组织学确诊乳头状）；
  PTC=-1 若 FNAC 提示乳头状（"papillary" in FNAC）但组织学未确认 -> 标签不确定，排除避免噪声；
  PTC=0  其余患者中，labels_orig 标为良性 lanhtinh 者（良性必然不是乳头状，可靠负类）；
  PTC=-1 其余（未知），训练时排除，避免标签噪声。
注意：这只是让代码“现在就能跑”的默认标签。后续别人区分好的标签请直接
覆盖 ptc_labels.csv（保持列：patient_id,ptc，ptc∈{-1,0,1}）即可被自动读取。
（若想恢复“组织学+FNAC 共同判定”，请用备份 build_ptc_labels_fnac.py。）
"""
import os
import csv
import json
...
def build_ptc_labels(data_root, out_csv):
    ...
    for pid, rec in pats.items():
        ...
        # 仅依据组织学 Histopathology 判定乳头状，不使用 FNAC
        is_ptc = ("papillary" in hist and "thyroid" in hist)
        fna_malignant = "papillary" in fna  # FNAC 提示乳头状（如 Bethesda VI）
        if is_ptc:
            ptc = 1
        elif fna_malignant:
            # 仅 FNAC 提示恶性、组织学未确认 -> 标签不确定，排除以免污染负类
            ptc = -1
