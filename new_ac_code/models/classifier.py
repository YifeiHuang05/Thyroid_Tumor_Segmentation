"""
任务 C 三分类器：输入由任务 A 的 mask 裁剪出的结节 patch（patch_size×patch_size RGB），
输出 C 个类别的 logits（softmax 后为各类概率；C=3：0=良性,1=恶性乳头状,2=恶性非乳头状）。
...
class NoduleClassifier(nn.Module):
    ...
    def __init__(self, base=32, num_classes=3):
        super().__init__()
...
def build_classifier(patch_size=224, backbone="efficientnet_b3", pretrained=True,
                     use_timm=True, num_classes=3):
    if use_timm:
        try:
            import timm
            model = timm.create_model(backbone, pretrained=pretrained, num_classes=num_classes)
            print(f"[classifier] 使用 timm backbone={backbone} num_classes={num_classes} pretrained={pretrained}")
            return model
        except Exception as e:
            print(f"[classifier] timm 不可用（{e}），回退到内置 CNN")
    return NoduleClassifier(num_classes=num_classes)
